...
class Normalizer:
    ...
    def normalize(self, rel_result: RelationResult) -> NormalizedPatientProfile:
        ...
        profile.bmi = self._normalize_bmi(rel_result.by_factor("bmi"))
    ...
    def _normalize_bmi(self, relations: List[Relation]) -> BMIRecord:
        rec = BMIRecord()
        if not relations:
            return rec
        ...
        for r in relations:
            ...
            # Pick up class from flags if no numeric value
            for flag in r.flags:
